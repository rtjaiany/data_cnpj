...
def main():
    ...
    try:
        ...
        print(f"Error parsing reference month '{data_folder}': {e}")
        sys.exit(1)
    ...
    cur.execute("""
        ...
        FROM snapshots
        WHERE tabela = 'empresa' AND mes_referencia = %s::date
    """, (ref_month_date,))
    ...
    cur.execute("""
        INSERT INTO latest_state_empresa (cnpj_basico, razao_social, natureza_juridica, qualificacao_responsavel, capital_social, porte_empresa, ente_federativo_responsavel, is_deleted, last_updated_month, data_coleta)
        SELECT
            stg.cnpj_basico,
            stg.razao_social,
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_stg_estab_pk ON staging_estabelecimento(cnpj_basico, cnpj_ordem, cnpj_dv);")
    conn.commit()
    ...
    cur.execute("""
        SELECT
            ...
            COUNT(case when tipo_alteracao = 'UPDATE' then 1 end)
        FROM snapshots
        WHERE tabela = 'estabelecimento' AND mes_referencia = %s::date
    """, (ref_month_date,))
    ...
    cur.execute("""
        SELECT
            ...
            COUNT(case when tipo_alteracao = 'UPDATE' then 1 end)
        FROM snapshots
        WHERE tabela = 'socios' AND mes_referencia = %s::date
    """, (ref_month_date,))
    ...
    cur.execute("""
        ...
        SELECT
            ...
            stg.faixa_etaria, FALSE, %s::date, %s::timestamp
        FROM staging_socios stg
        LEFT JOIN latest_state_socios l ON l.cnpj_basico = stg.cnpj_basico AND l.nome_socio_razao_social = stg.nome_socio_razao_social
        LEFT JOIN socios b ON b.cnpj_basico = stg.cnpj_basico AND b.nome_socio_razao_social = stg.nome_socio_razao_social AND l.cnpj_basico IS NULL
        CROSS JOIN LATERAL (
    ...
    if soc_deletes > 0:
        cur.execute("""
            ...
            SELECT
                prev.cnpj_basico, prev.nome_socio_razao_social, TRUE, %s::date, %s::timestamp
            FROM (
    ...
    cur.execute("""
        SELECT
            ...
            COUNT(case when tipo_alteracao = 'UPDATE' then 1 end)
        FROM snapshots
        WHERE tabela = 'simples' AND mes_referencia = %s::date
    """, (ref_month_date,))
    ...
    cur.execute("""
        ...
        SELECT
            ...
            stg.data_opcao_mei, stg.data_exclusao_mei, FALSE, %s::date, %s::timestamp
        FROM staging_simples stg
        LEFT JOIN latest_state_simples l ON l.cnpj_basico = stg.cnpj_basico
        LEFT JOIN simples b ON b.cnpj_basico = stg.cnpj_basico AND l.cnpj_basico IS NULL
        CROSS JOIN LATERAL (
    ...
    cur.execute("""
        ...
        VALUES (%s::date, %s::timestamp, 'SUCCESS', %s, %s, %s, %s)
